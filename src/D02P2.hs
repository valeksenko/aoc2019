...
findnounverb :: [Register] -> Maybe Int
findnounverb registers = findTarget >>= Just . res 
    where
        findTarget = find ((==) target . execHead) [(i,j) | i <- [0..99], j <- [0..99]]
        execHead (noun, verb) = head . exec $ (take 1 registers) ++ [noun,verb] ++ (drop 3 registers)
        res (noun, verb) = 100 * noun + verb
{-

...
adventure :: [Register] -> IO ()
adventure = run . mkIntcode 'a' []

run :: State -> IO ()
run state = do
    let state' = runProgram state
    putStrLn . map chr . reverse $ sOutput state'

    cmd <- getLine
    if null cmd then return () else run $ state' { sOutput = [], sInput = (map ord cmd) ++ [10] }

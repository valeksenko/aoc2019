...
fftnumber :: Int -> String -> Integer
fftnumber numSteps = calc . concat . replicate 10000 . map (toInteger . digitToInt)
    where
        calc initial = unDigits 10 . take 8 . drop (skip initial) . last . take (numSteps + 1) $ iterate calcFFT initial
        skip = fromIntegral . unDigits 10 . take 7
